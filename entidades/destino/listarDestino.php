...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-6 col-md-offset-3">
      <h1 class="text-center">Lista de Destinos</h1>
    </div>
  </div>
  <hr>
...
<?Php
  ...
  $con=Conectar();
  $sql=$con->prepare('select * from destino'); //Se prepara la sentencia SQL
  ...
  foreach($resultado as $row)
  {
   // echo("<tr> <td>". $row["per_ced"]."</td><td>".$row["per_nombre"].  " </td><td> ".$row["per_apellido"]."</td><td>".$row["per_telefono"]."</td><td>".$row["per_direccion"]."</td><td>".$row["per_ciudad"]."</td><td>".$row["per_email"]."</td><td><a href='up.php?id=".$row["per_id"]."  '>Eliminar</a></td><td><a href='eli.php?id=".$row["per_id"]."'>Eliminar</a></td></tr>");
    echo("<tr>
            <td>".$row["des_id"]."</td>
            <td>".$row["des_descripcion"]."</td>
            <td><a href='actualizarDestino.php?
            id=".$row["des_id"].
            ...
            <td><a href='eliminarDestino.php?id=".$row["des_id"]."  '>
